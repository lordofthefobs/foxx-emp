<?php
	include "util/sql_queries.php";
	$q = new queries();
	date_default_timezone_set("US/Central");
?>
<script type="text/javascript">
	function update(emp_id){
		// alert("BOOOOO");
		var salary = document.getElementById('salary_' + emp_id).value;
		var bonus = document.getElementById('bonus_'+emp_id).value;
		var other_ded = document.getElementById('other_ded_'+emp_id).value;

		if( isNaN(salary)){
			alert("Please Enter a number for Salary");
		}else if( isNaN(bonus)){
			alert("Please Enter a number for Bonus");
		}else if( isNaN(other_ded)){
			alert("Please Enter a number for Other Deductions");
		}else{
			salary = parseFloat(salary);
			bonus = parseFloat(bonus);
			other_ded = parseFloat(other_ded);
		
			var total_tax = parseFloat(document.getElementById('total_tax_'+emp_id).value);
			var total_ded = parseFloat(document.getElementById('total_ded_'+emp_id).value);
		
			var gross_pay = document.getElementById('gross_pay_'+emp_id);
			var taxes = document.getElementById('taxes_'+emp_id);
			var net_pay = document.getElementById('net_pay_'+emp_id);
		
			var tax = salary * total_tax / 100;
			gross_pay.innerHTML = (salary + bonus).toFixed(2);
			taxes.innerHTML = (tax + total_ded).toFixed(2);
			net_pay.innerHTML = ((salary+bonus) - (tax + total_ded + other_ded)).toFixed(2);
		}
	}
	function check_date(t){
		if( t == 'm'){
			var month = parseInt(document.salary_form.month.value);
			if( month <= 0 || month > 12){
				alert( "Please Enter valid month");
			}
		}else if(t == 'd'){
			var day = parseInt(document.salary_form.day.value);

			if( day <= 0 || day > 31){
				alert("Please Enter a valid day");
			}
		}else if(t == 'y'){
			var year = parseInt(document.salary_form.year.value);
			if( year <= 2000 || year > 2099){
				alert("Please Enter a valid year");
			}
		}
	}
</script>
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>Foxx Beauty Employee Management System - Salary Payroll Calculator</title>

</head>

<body>
	<table width="100%" border="0" bgcolor="#3B5998" height="50">
	  <tr>
	    <td width="38">&nbsp;</td>
	    <td width="368"><font size="10" color="#FFFFFF">Foxx Beauty</font></td>
	    <td width="449"></td>
	  </tr>
	</table>
<?php
	$emp_data = $q->get_salary_employees();
?>
<p>

<form name ="salary_form" action="Paystubs.php" method="post">
	<center>
		<table width="100%" border="1" frame="border" cellpadding='0' cellspacing='0'>
			<tr>
				<td width="5" colspan=2>Period Ending in:</td>
				<td colspan=8><input type="text" name="month" size=2 value='<?php echo date("m")?>' onChange="check_date('m');">
					/ <input type="text" name="day" size=2 value='<?php echo date("d")?>' onChange="check_date('d')">
					/ <input type="text" name="year" size=4 value='<?php echo date("Y")?>' onChange="check_date('y')"></td>
			</tr>
			<tr></tr>
			<tr>
				<td width="10">ID</td>
				<td width="10">Name</td>
				<td width="10">Salary</td>
				<td width="10">Bonus</td>
				<td width="10">Gross Pay</td>
				<td width="10">Taxes and<br>Deductions</td>
				<td width="10">Other<br>Deductions</td>
				<td width="10">Net Pay</td>
				<td width="10">Note</td>
			</tr>
			<?php
				foreach($emp_data as $e){
					$total_tax = $e[7] + $e[8] + $e[9];
					$total_ded = $e[10] + $e[11] + $e[12];
					$emp_id = $e[0];
					?>
						<tr>
							<input type='hidden' name='total_tax_<?php echo $emp_id;?>' id='total_tax_<?php echo $emp_id;?>' value='<?php echo $total_tax;?>'>
							<input type='hidden' name='total_ded_<?php echo $emp_id;?>' id='total_ded_<?php echo $emp_id;?>' value='<?php echo $total_ded;?>'>
							<td width="10"><?php echo $emp_id?></td>
							<td width="10"><?php echo $e[1]?></td>
							<td width="10"><input type='text' name='salary_<?php echo $emp_id;?>' id='salary_<?php echo $emp_id;?>' onChange="update(<?php echo $emp_id;?>);" value=0.00></td>
							<td width="10"><input type='text' name='bonus_<?php echo $emp_id;?>' id='bonus_<?php echo $emp_id;?>' onChange="update(<?php echo $emp_id;?>);" value=0.00></td>
							<td width="10"><label name='gross_pay_<?php echo $emp_id;?>' id='gross_pay_<?php echo $emp_id;?>'>0.00</label></td>
							<td width="10"><label name='taxes_<?php echo $emp_id;?>' id='taxes_<?php echo $emp_id;?>'>0.00</label>
							
							<td width="10"><input type='text' name='other_ded_<?php echo $emp_id;?>' id='other_ded_<?php echo $emp_id;?>' onChange="update(<?php echo $emp_id;?>);" value=0.00></td>
							<td width="10"><label name='net_pay_<?php echo $emp_id;?>' id='net_pay_<?php echo $emp_id;?>'>0.00</label></td>
							<td width="10"><input type='text' name='note_<?php echo $emp_id;?>' value=" "></td>
						</tr>
					<?php
				}
			?>
			<tr>
				<td colspan=2><input type="submit" name="salary" value="Submit"></td>

				<td colspan=6 border=0></td>
				<td width="10">
				<center>
					<input type='button' name="to_main" value="Back to Main" onClick="document.location='./Emp.php';">
				</center>
				</td>
			</tr>
		</table>
	</center>
</form>
</body>
</html>
